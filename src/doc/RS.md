...
- **game/camera.rs** — `update_camera`: позиция камеры (только в режиме `move`,
  drag мышью/пальцем или стрелки), ограничение видимой областью карты.
...
- **mod.rs** — объявляет `camera`, `interact`, `cursor`; `do_input` — главная
  ...
  Tab — смена режимов (move/interact/build/delete), валидность и превью, возврат
  слотов/режима/зума.
...
- **cursor.rs** — `handle_mouse_movement` (NDC→мир→клетка, клампинг, задержка между
  ...
  `update_cursor_preview` (превью расстановки); режим `move` оставляет карту
  без действия и включает перемещение камеры.
