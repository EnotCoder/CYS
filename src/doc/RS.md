...
- **group.rs** — многоклеточные объекты: `add_group_object` (на каждый тайл —
  ...
  `find_group_at_position` (декор приоритетнее ковра).
- **placement.rs** — `can_place_at`: проверка размещения — категории требуют
  подходящие клетки (ковры/декор — пол, wall decor — стены, outdoor/цветы — трава),
  запрет класть поверх занятой клетки.
