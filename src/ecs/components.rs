...
/// Ошибка размещения объекта: если игрок попытался поставить объект,
/// но не хватило денег, сюда записывается (текущие деньги, цена объекта).
/// Сцена читает ресурс за кадр и показывает красную подсказку.
pub struct PlacementError(pub Option<(i32, i32)>);

/// Какие «необычные» (светящиеся) предметы куплены в текущем мире.
/// Хранится в сохранении мира; размещать такие объекты можно только
/// после покупки доступа в магазине.
pub struct ShopOwned(pub Vec<String>);

/// Флаг: игрок попытался поставить светящийся предмет, не купив доступ.
/// Сцена показывает красную подсказку «Buy this in the Shop».
pub struct ShopDenied(pub bool);
